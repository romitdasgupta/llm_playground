...
class GenerateRequest(BaseModel):
    ...
    temperature: float = Field(
        default=0.8, ge=0.1, le=2.0, description="Temperature for sampling"
    )
    max_length: int = Field(
        default=100, ge=20, le=500, description="Maximum length of generated text"
    )
    top_k: Optional[int] = Field(
        default=50, ge=1, le=100, description="Top-k value for top-k sampling"
    )
    top_p: Optional[float] = Field(
        default=0.95, ge=0.0, le=1.0, description="Top-p value for nucleus sampling"
    )
    num_beams: Optional[int] = Field(
        default=5, ge=1, le=10, description="Number of beams for beam search"
    )
...
async def generate_text(request: GenerateRequest):
    ...
    try:
        ...
        if request.strategy == "greedy":
            ...
            strategy_params = {"type": "beam_search", "num_beams": request.num_beams}
